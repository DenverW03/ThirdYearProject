...
def world_setup(numRobots):
    ...
    data[13] = "    ConvoyRobot *robots = (ConvoyRobot *) malloc(" + str(numRobots) + " * sizeof(ConvoyRobot));\r\n"

    # Aadd the VIP robot spawning
    data[13] += "    VipRBT::VipRobot vip = VipRBT::VipRobot((ModelPosition *)world.GetModel(argv[2]), Pose(-8, 8, 0, 0));\r\n"

    # Add convoy robot spawning lines
    for i in range(numRobots):
        data[13] = data[13] + '    robots[' + str(i) + '] = ConvoyRobot((ModelPosition *)world.GetModel(argv[' + str(i + 3) + ']), Pose::Random(-12, 12, -12, 12), ' + str(i + 1) + ');\r\n'
