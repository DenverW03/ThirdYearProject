...
def world_setup(num_robots, directory):
    ## WORLDFILE WRITING
...
def test_set(num_robots, params, string, directory):
    ...
    world_setup(num_robots, directory)
    run_simulation(directory)

    os.chdir(this_path)

def test_all_sets(sets, params, string, algorithm_dir):
    # Testing the ring based algorithm
    directory = "../" + algorithm_dir + "/"
    data_directory = "./" + algorithm_dir + "_results/"
    for num_robots in sets:
        for i in range(runs):
            test_set(num_robots, params, string, directory)
            subprocess.call(["pkill", "-f", "generated.world"])
        ...
        os.remove(data_directory + "output.csv")
